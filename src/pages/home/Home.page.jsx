...
import UploaderHomeCard from "./UploaderHomeCard.component.jsx";
import RightCommentCard from "./RightCommentCard.component.jsx";
...
const Home = () => {
  ...
  return (
    <div className="overscroll-none lg:flex">
      ...
      <div className="lg:columns-2 lg:gap-0 pt-4">
        {posts.map((item, i) => 
          <div className="shrink-0 my-4 break-inside-avoid"> 
            <PostCard username={item.username} avatar={item.avatar} timeAgo={item.timeAgo} title={item.title} postImage={item.postImage} likeCount={item.likeCount} commentCount={item.commentCount} shareCount={item.shareCount} />
          </div>)}
      </div>
    </div>
  )
